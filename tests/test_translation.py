...
from recipe_bot.translation import LibreTranslateTranslator, metric_temperatures
...
class TranslatorTests(unittest.IsolatedAsyncioTestCase):
    async def test_http_contract_and_numeric_validation(self):
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
            result = await LibreTranslateTranslator(client, "http://libretranslate:5000", "de", 30).translate(
                ["Rice", "100 g rice"]
            )
    ...
    async def test_invalid_or_partial_responses_and_errors_are_safe(self):
        responses = [
            {"translatedText": ["Reis"]},
            {"translatedText": "Reis"},
            {"translatedText": ["Reis", "200 g Reis"]},
        ]
            ...
            with self.subTest(data=data):
                async with httpx.AsyncClient(transport=httpx.MockTransport(
                    ...
                    with self.assertRaises(APIError):
                        await LibreTranslateTranslator(client, "http://libretranslate:5000", "de", 3).translate(
                            ["Rice", "100 g rice"]
                        )
        async with httpx.AsyncClient(transport=httpx.MockTransport(
            ...
            with self.assertRaises(APIError) as raised:
                await LibreTranslateTranslator(client, "http://libretranslate:5000", "de", 3).translate(["Rice"])
...
class TranslationServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_metric_recipe_translates_without_changing_english_fallback(self):
        converted = recipe()
    ...
    async def test_long_recipe_batches_ingredients_then_instructions(self):
        ...
        field, texts = self.service.translation_batch(session)
        self.assertEqual((field, texts),
                         ("translated_ingredients", ["125 g flour", "200 g sugar"]))
    ...
    async def test_failed_metric_translation_returns_exact_english_original(self):
        ...
        self.api.recipes = AsyncMock(return_value=[converted, recipe(2)])
        await self.menu()
        self.translator.translate.side_effect = APIError("LibreTranslate validation")
        await self.select()
    ...
    async def test_menu_retries_then_english_and_no_extra_fetch(self):
        self.translator.translate.side_effect = APIError("LibreTranslate translation")
        await self.service.tick()
        worker = await self.start_worker()
        await self.wait_for(lambda: self.service.state.session.translation_retry_at > self.now)
    ...
    async def test_disabling_translation_resolves_pending_menu_without_requests(self):
        await self.service.tick()
        self.service = RecipeService(self.settings, self.store, self.api, self.telegram,
                                     clock=lambda: self.now, dashboard=self.dashboard)
        await self.start_worker()
    ...
    async def test_disabling_translation_resolves_pending_recipe_without_requests(self):
        worker = await self.menu()
        await self.stop_worker(worker)
        await self.select()
        self.service = RecipeService(self.settings, self.store, self.api, self.telegram,
    ...
    def test_optional_url_and_invalid_configuration(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict("os.environ", required, clear=True):
                ...
                self.assertIsNone(credentials.libretranslate_url)
                self.assertEqual(settings.translation_request_timeout_seconds, 30)
                self.assertEqual(settings.translation_job_timeout_minutes, 15)
            with patch.dict("os.environ", {**required, "LIBRETRANSLATE_URL": "http://libretranslate:5000"},
                            clear=True):
                _, credentials = load_config(path, Path(directory) / ".env")
                self.assertEqual(credentials.libretranslate_url, "http://libretranslate:5000")
            for url in ("ftp://libretranslate", "http://user:pass@libretranslate",
                        "http://libretranslate/translate", "http://libretranslate?key=secret"):
                with self.subTest(url=url), patch.dict("os.environ", {**required, "LIBRETRANSLATE_URL": url},
                                                       clear=True):
                    with self.assertRaises(ValueError):
                        load_config(path, Path(directory) / ".env")
        for kwargs in ({"translation_attempts": 0}, {"translation_request_timeout_seconds": 0},
